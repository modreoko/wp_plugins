...
function admin_event_list_row_actions( $actions, $post ){
    ...
    if ( $post->post_type == "event-recurring" ) {
        ...
        if( $flk_event->getEnabledSessionRegistration() || $flk_event->getSessionRegistrationCount() > 0){
            ...
            $url = admin_url( 'edit.php?post_type=event-recurring' );
...
function flk_entry_field_content( $content, $field, $value, $lead_id, $form_id ) {
    ...
    if ( RG_CURRENT_VIEW == 'entry' ) {
        if( $mode == 'view' ){
            ...
            $ignored_fields = array( 'location_sessions', 'location_name' );
            if( in_array($field->inputName, $ignored_fields) ){
                return null;
            }
        } else{
            
            // d not display some fields in edit screen
            $ignored_fields = array( 'secure_key', 'session_name', 'location_name' );
            if( in_array($field->inputName, $ignored_fields) ){
